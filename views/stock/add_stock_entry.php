                                ...
                                <div class="box-body">
                                    ...
                                    <form action="<?=base_url('stock/SaveStockEntry/');?>" method="post">
                                        ...
                                        <div class="form-group col-md-6">
                                            <label>GRN Number <span style="color:red;">*</span></label>
                                            <select class="form-control grn_number" name="grn_number">
                                                <option value="">Select GRN Number</option>
                                                <?php foreach( $grns as $grn ) : ?>
                                                    <option <?php if( isset($_GET['grn']) && $_GET['grn'] == $grn['grn_id'] ) { echo "selected"; } ?> value="<?=$grn['grn_id'];?>"><?=$grn['grn_number'];?></option>
                                                <?php endforeach; ?>
                                            </select>  
										...
										<div class="form-group col-md-6">
											...
											<?php if( isset($_GET['grn']) ) : $grnid = $_GET['grn']; endif; ?> 
											<select class="form-control item" name="item" style="width:100%;">
												<option value=''>Select Items</option>
												<?php 	if( $grnid ) :
												
															$item_ids = $this->db->get_where('grn_item',array('grn_row_id'=>$grnid))->result_array();
															 
															foreach($item_ids as $item_id)
																{
																	echo '<option value="'.$item_id['item_id'].'">'.GetITEMdata($item_id['item_id'])->ITEM_CODE.'</option>';
																}
														endif; ?>
											</select>
										...
										<div class="form-group col-md-6">
											...
											<select class="form-control allotedQTY" name="qty" style="width:100%;">
		...
		<script type="text/javascript">
            ...
            $(document).ready(function() {
                $(".item").change(function(){
                    ...
                    var itemID = $(this).val();
                    var grnID = $('.grn_number').val();
                    //alert(grn_id);   
                    $.ajax({
                        url: '/stock/ItemQtyGRN',
                        data: {'itemID': itemID, 'grnID': grnID},
                        type: "post",
                        success: function(data){
							//alert(data);
                            $('.allotedQTY').removeAttr('disabled');
                            $(".allotedQTY").select2("destroy");
                            $('.allotedQTY').html(data);
                            $(".allotedQTY").select2();
                            //alert(data);
                        }
                    });
